...
bool get_gps_position(char *buffer, int buffer_len){
    // Stringhe di test fisse
    const char *test_lat = "45.123456";
    const char *test_lon = "9.654321";

    // Format: LAT:45.123456,LON:9.654321
    int written = snprintf(buffer, buffer_len, "LAT:%s,LON:%s", test_lat, test_lon);

    // Se scritto con successo e non troncato
